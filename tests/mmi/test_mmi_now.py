...
from tickersnap.mmi.mmi import MMINow
    ...
    def test_api_response_structure_validation(self):
        ...
        with MMINow() as mmi:
            with patch.object(mmi.client, 'get') as mock_get:
                ...
                assert hasattr(data, 'current_value') and isinstance(data.current_value, float)
                assert hasattr(data, 'daily') and isinstance(data.daily, list)
                
                # validate historical comparison fields
                assert hasattr(data, 'last_day') and isinstance(data.last_day, HistoricalData)
                assert hasattr(data, 'last_week') and isinstance(data.last_week, HistoricalData)
                assert hasattr(data, 'last_month') and isinstance(data.last_month, HistoricalData)
                assert hasattr(data, 'last_year') and isinstance(data.last_year, HistoricalData)
                
                # validate daily data structure
                if data.daily:
                    daily_item = data.daily[0]
                    assert isinstance(daily_item, DailyData)
                    assert hasattr(daily_item, 'value') and isinstance(daily_item.value, float)
                    assert hasattr(daily_item, 'date') and isinstance(daily_item.date, datetime)
    ...
    def test_api_response_data_integrity(self):
        ...
        with MMINow() as mmi:
            with patch.object(mmi.client, 'get') as mock_get:
                ...
                assert data.last_week.indicator == expected_data["lastWeek"]["indicator"]
                assert data.last_month.indicator == expected_data["lastMonth"]["indicator"]
                assert data.last_year.indicator == expected_data["lastYear"]["indicator"]
    ...
    def test_tickertape_now_api_call_validation(self):
        ...
        with MMINow(timeout=30) as mmi:
            try:
                ...
                assert isinstance(result, MMINowResponse), "Response should be MMINowResponse"
                ...
                assert isinstance(data.current_value, float), f"Current value should be float, got {type(data.current_value)}"
                assert 0 <= data.current_value <= 100, f"Current value should be between 0-100, got {data.current_value}"
                
                # validate historical comparison fields
                assert hasattr(data, 'last_day'), "Missing 'last_day' field"
                assert isinstance(data.last_day, HistoricalData), f"Last day should be HistoricalData, got {type(data.last_day)}"
                
                assert hasattr(data, 'last_week'), "Missing 'last_week' field"
                assert isinstance(data.last_week, HistoricalData), f"Last week should be HistoricalData, got {type(data.last_week)}"
                
                assert hasattr(data, 'last_month'), "Missing 'last_month' field"
                assert isinstance(data.last_month, HistoricalData), f"Last month should be HistoricalData, got {type(data.last_month)}"
                
                assert hasattr(data, 'last_year'), "Missing 'last_year' field"
                assert isinstance(data.last_year, HistoricalData), f"Last year should be HistoricalData, got {type(data.last_year)}"
                
                # validate daily data array
                assert hasattr(data, 'daily'), "Missing 'daily' field"
                assert isinstance(data.daily, list), f"Daily should be list, got {type(data.daily)}"
                assert len(data.daily) > 0, "Daily array should not be empty"
                
                # validate daily data structure
                daily_item = data.daily[0]
                assert isinstance(daily_item, DailyData), f"Daily item should be DailyData, got {type(daily_item)}"
                assert hasattr(daily_item, 'value'), "Daily item missing 'value' field"
                assert isinstance(daily_item.value, float), f"Daily value should be float, got {type(daily_item.value)}"
                assert hasattr(daily_item, 'date'), "Daily item missing 'date' field"
                assert isinstance(daily_item.date, datetime), f"Daily date should be datetime, got {type(daily_item.date)}"
                ...
                assert abs(data.current_value - data.indicator) < 0.01, f"Current value ({data.current_value}) should match indicator ({data.indicator})"
                
                print(f"✅ Integration test passed! Current MMI: {data.current_value:.2f}, Nifty: {data.nifty:.2f}, Gold: {data.gold}")
                print(f"   Historical MMI - Day: {data.last_day.indicator:.2f}, Week: {data.last_week.indicator:.2f}, Month: {data.last_month.indicator:.2f}, Year: {data.last_year.indicator:.2f}")
