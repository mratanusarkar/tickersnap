...
from tickersnap.mmi.models import (
    MMICurrent, MMITrends, MMIChanges, MMIDataPoint, MMIZone,
    MMINowData, MMIPeriodData, MMINowResponse, MMIPeriodResponse
)
...
class TestUnitMarketMoodIndex:
    ...
    def test_get_current_mmi_zone_calculations(self, mock_mmi_now_class):
        ...
        for mmi_value, expected_zone in test_cases:
            ...
            result = mmi.get_current_mmi()
            assert result.zone == expected_zone, f"MMI {mmi_value} should be {expected_zone}, got {result.zone}"
    ...
    def test_timeout_parameter_propagation(self):
        ...
        with patch('tickersnap.mmi.mmi.MMINow') as mock_now:
            with patch('tickersnap.mmi.mmi.MMIPeriod') as mock_period:
                # Setup mocks
                mock_now.return_value.__enter__.return_value.get_data.return_value = self._create_mock_now_response()
...
class TestIntegrationMarketMoodIndex:
    ...
    def test_get_current_mmi_integration(self):
        ...
        try:
            ...
            assert result.zone in [MMIZone.EXTREME_FEAR, MMIZone.FEAR, MMIZone.GREED, MMIZone.EXTREME_GREED]
    ...
    def test_get_mmi_trends_integration(self):
        ...
        try:
            ...
            print(f"✅ Trends - Current: {result.current.value:.2f}, Days: {len(result.last_10_days)}, Months: {len(result.last_10_months)}")
    ...
    def test_get_mmi_changes_integration(self):
        ...
        try:
            ...
            for point in [result.current, result.last_day, result.last_week, result.last_month, result.last_year]:
                assert 0 <= point.value <= 100
                assert isinstance(point.date, datetime)
            
            # Validate property calculations
            assert abs(result.vs_last_day - (result.current.value - result.last_day.value)) < 0.01
            assert abs(result.vs_last_week - (result.current.value - result.last_week.value)) < 0.01
            assert abs(result.vs_last_month - (result.current.value - result.last_month.value)) < 0.01
            assert abs(result.vs_last_year - (result.current.value - result.last_year.value)) < 0.01
            ...
            print(f"   vs Day: {result.vs_last_day:+.1f}, vs Week: {result.vs_last_week:+.1f}")
            print(f"   vs Month: {result.vs_last_month:+.1f}, vs Year: {result.vs_last_year:+.1f}")
    ...
    def test_all_methods_consistency(self):
        ...
        try:
            ...
            print(f"✅ Consistency check passed - all methods return similar current values")
            print(f"   Current: {current.value:.2f}, Trends: {trends.current.value:.2f}, Changes: {changes.current.value:.2f}")
